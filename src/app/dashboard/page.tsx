...
export default function DashboardPage() {
  ...
  // lock to 90% zoom like browser 90% (user preference) — desktop only; mobile uses native viewport
  React.useEffect(() => {
  ...
  return (
    <div className="flex min-h-[100dvh] lg:min-h-[111.11vh] w-full bg-[#f8f8f5] dark:bg-[#070d07]">
      {/* LEFT PANEL - desktop */}
      <aside className="hidden lg:flex w-[300px] shrink-0 flex-col border-r border-[var(--border)] bg-white dark:bg-[#0f1410] sticky top-0 h-[100dvh] lg:h-[111.11vh]">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-[4px] px-3 py-3 text-left text-[15px] transition-colors ${active ? "bg-[var(--off-white)] dark:bg-white/5 font-semibold text-[var(--text-dark)]" : "text-[var(--text-mid)] hover:bg-[var(--off-white)] dark:hover:bg-white/5"}`}
                >
        ...
        <>
          ...
          <div className="fixed inset-y-0 left-0 z-50 flex w-[300px] max-w-[86vw] flex-col border-r border-[var(--border)] bg-white dark:bg-[#0f1410] shadow-2xl lg:hidden animate-[toast-in_0.22s_ease] overflow-hidden">
            ...
            <nav className="flex-1 overflow-auto p-4 space-y-6">
              <div className="space-y-2">
                {[
                  ...
                  return (
                    <button
                      key={item.label}
                      onClick={() => { setFilter(item.filter as any); setMobileNavOpen(false); }}
                      className={`flex w-full items-center gap-3 rounded-[4px] px-3 py-3.5 text-left text-[15px] transition-colors touch-manipulation ${active ? "bg-[var(--off-white)] dark:bg-white/5 font-semibold text-[var(--text-dark)]" : "text-[var(--text-mid)] hover:bg-[var(--off-white)] dark:hover:bg-white/5"}`}
                    >
              ...
              <button onClick={() => toast("Export CSV - coming soon","info")} className="w-full cursor-pointer rounded-[4px] border border-[var(--border)] bg-white dark:bg-white/5 px-4 py-3.5 text-[16px] font-medium text-[var(--text-dark)] hover:border-[#4a8c3f] touch-manipulation">Export submissions</button>
            </nav>
            <div className="p-4 border-t border-[var(--border)] shrink-0">
              <button onClick={() => { setProfileModalOpen(true); setMobileNavOpen(false); }} className="flex w-full items-center gap-3 rounded-[4px] px-2 py-2.5 hover:bg-[var(--off-white)] dark:hover:bg-white/5 transition-colors text-left cursor-pointer touch-manipulation">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-[#1a3a1a] to-[#4a8c3f] text-white font-bold text-[18px]">
                  {avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" /> : (displayName || userName || "A")[0]?.toUpperCase() || "A"}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[16px] font-bold leading-none text-[var(--text-dark)]" style={{ fontFamily: "Roboto, sans-serif" }}>{displayName || (userName !== "there" ? userName : "Administrator")}</p>
                  <p className="text-[13px] text-[var(--text-light)] truncate">{emailField || userEmail}</p>
                </div>
                <Settings className="h-4 w-4 text-[var(--text-light)] shrink-0" />
              </button>
            </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex h-[64px] items-center justify-between gap-2 sm:gap-4 px-3 sm:px-6 lg:px-8 border-b border-[var(--border)] bg-white dark:bg-[#1a221a] sticky top-0 z-20">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <h1 className="text-[15px] sm:text-[18px] font-bold text-[var(--text-dark)] truncate">Repository Dashboard</h1>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button onClick={signOut} className="group inline-flex cursor-pointer items-center gap-2 rounded-[4px] border border-[var(--border)] bg-white dark:bg-white/5 px-3 sm:px-4 py-2.5 text-[14px] font-bold text-[var(--text-dark)] hover:border-[#4a8c3f] hover:text-[#4a8c3f] hover:shadow-md active:translate-y-0 transition-all touch-manipulation">
              <LogOut className="h-4 w-4 shrink-0" /> <span className="hidden sm:inline">Sign out</span><span className="sm:hidden sr-only">Sign out</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 bg-[#f8f8f5] dark:bg-[#070d07] overflow-auto">
          {/* CONTROL HUB */}
          <div className="rounded-[16px] border border-[var(--border)] bg-gradient-to-br from-[#1a3a1a] via-[#2d5a27] to-[#4a8c3f] p-5 sm:p-7 lg:p-8 text-white shadow-lg">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 sm:gap-6">
              <div className="flex-1 min-w-0">
                <p className="text-[11px] sm:text-[13px] font-bold tracking-[0.14em] uppercase text-white/60">Control Hub</p>
                <h2 className="mt-2 flex flex-wrap items-center gap-2 text-[22px] sm:text-[26px] lg:text-[28px] font-bold leading-tight">
                ...
                <p className="mt-2 max-w-2xl text-[13px] sm:text-[14px] font-light leading-6 text-white/80">Review open submissions, publish to the four collections, and keep the repository living - not archived.</p>
              </div>
              <div className="flex flex-col sm:flex-row lg:flex shrink-0 gap-2 sm:gap-3 w-full lg:w-auto">
                <Link href="/submit" className="inline-flex items-center justify-center rounded-[4px] bg-white px-6 py-3.5 sm:py-3 text-[14px] sm:text-[15px] font-bold text-[#1a3a1a] hover:bg-white/90 transition-all w-full sm:w-auto touch-manipulation min-h-[44px]">New submission</Link>
                <Link href="/collections" className="inline-flex items-center justify-center rounded-[4px] border border-white/20 px-6 py-3.5 sm:py-3 text-[14px] sm:text-[15px] font-bold text-white hover:bg-white/10 transition-colors w-full sm:w-auto touch-manipulation min-h-[44px]">Browse</Link>
              </div>
            </div>
          </div>
            ...
            {[
              ...
              return (
                <motion.div
                  ...
                  className={`h-full overflow-hidden rounded-[12px] border border-[var(--border)] bg-white dark:bg-[#1a221a] ${c.lightBg} p-3 sm:p-4`}
                >
                  <div className="p-0.5">
                    ...
                    <div className="mb-3 flex items-baseline gap-1.5 sm:gap-2">
                      <span className="text-[22px] sm:text-[30px] font-bold leading-none tracking-tight text-[var(--text-dark)]" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
          ...
          {/* CONTROLS */}
          <div className="flex flex-col gap-3 sm:gap-4 sm:flex-row items-stretch sm:items-center justify-between rounded-[16px] border border-[var(--border)] bg-white dark:bg-[#1a221a] p-3 sm:p-4 shadow-sm">
            <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-1 px-1 sm:flex-wrap sm:overflow-visible">
              {(["all", "pending", "in_review", "published"] as const).map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`shrink-0 whitespace-nowrap rounded-[4px] px-4 sm:px-5 py-3 sm:py-2.5 text-[12px] sm:text-[13px] font-bold uppercase tracking-wide transition-all touch-manipulation min-h-[44px] sm:min-h-0 ${filter === f ? "bg-[#1a3a1a] text-white shadow" : "bg-[var(--off-white)] dark:bg-white/5 text-[var(--text-mid)] hover:bg-[var(--border)]"}`}>
                  {f.replace("_", " ")}
                </button>
              ))}
            </div>
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search title or author…" className="w-full sm:w-72 rounded-[4px] border border-[var(--border)] bg-[var(--off-white)] dark:bg-white/5 px-4 sm:px-5 py-3.5 sm:py-3 text-[15px] outline-none focus:border-[#4a8c3f] focus:ring-4 focus:ring-[#4a8c3f]/10 transition-all min-h-[44px]" />
          </div>
          ...
          <div className="rounded-[16px] border border-[var(--border)] bg-white dark:bg-[#1a221a] overflow-hidden shadow-sm">
            <div className="flex items-center justify-between border-b border-[var(--border)] px-4 sm:px-6 py-4 sm:py-5 gap-2">
              <h3 className="text-[15px] font-bold text-[var(--text-dark)] shrink-0">Submissions</h3>
              <span className="text-[12px] sm:text-[13px] font-medium text-[var(--text-light)] truncate">{filtered.length} items • tap to view</span>
            </div>
              ...
              {filtered.map((s, idx) => (
                <div key={s.id} onClick={() => setSelected(s)} className="group flex flex-col gap-3 px-4 sm:px-6 py-4 sm:py-5 hover:bg-[var(--off-white)]/70 dark:hover:bg-white/[0.03] cursor-pointer transition-colors" style={{ animation: `toast-in 0.4s ease ${idx * 40}ms both` }}>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-1.5 text-[13px] sm:text-[14px] font-medium text-[var(--text-light)] line-clamp-1 break-all">{s.collection} • {s.author}{s.authorEmail ? ` (${s.authorEmail})` : ""} • {s.dateTime || s.date}</p>
                  ...
                  <div className="flex flex-wrap gap-2 w-full sm:w-auto sm:justify-end">
                    ...
                    {s.status !== "published" && s.status !== "declined" && (
                      <>
                        <button onClick={(e) => { e.stopPropagation(); act(s.id, "published"); }} className="flex-1 sm:flex-none rounded-[4px] bg-[#4a8c3f] px-4 py-3 sm:py-2.5 text-[14px] font-bold text-white hover:bg-[#2d5a27] hover:shadow transition-all touch-manipulation min-h-[44px] sm:min-h-0 text-center">Publish</button>
                        <button onClick={(e) => { e.stopPropagation(); setDeclineTarget(s); setDeclineNotes(""); }} className="flex-1 sm:flex-none rounded-[4px] border border-red-200 bg-red-50 px-4 py-3 sm:py-2.5 text-[14px] font-bold text-red-700 hover:bg-red-100 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300 transition-colors touch-manipulation min-h-[44px] sm:min-h-0 text-center">Decline</button>
                      </>
                    )}
                    {s.status === "declined" && <button onClick={(e) => { e.stopPropagation(); act(s.id, "pending"); }} className="flex-1 sm:flex-none rounded-[4px] border border-[var(--border)] px-4 py-3 sm:py-2.5 text-[14px] font-bold hover:border-amber-300 transition-colors touch-manipulation min-h-[44px] sm:min-h-0">Restore</button>}
                  </div>
                </div>
              ))}
              {filtered.length === 0 && <div className="px-4 sm:px-6 py-16 text-center text-[15px] sm:text-[16px] text-[var(--text-light)]">No submissions match your filter.</div>}
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="relative w-full max-w-[560px] max-h-[92dvh] sm:max-h-[90vh] overflow-auto rounded-t-[16px] sm:rounded-[16px] bg-white dark:bg-[#1a221a] border border-[var(--border)] shadow-2xl animate-[toast-in_0.3s_ease] pb-[env(safe-area-inset-bottom)]">
            ...
            <div className="p-4 sm:p-6 space-y-6">
              ...
              <div className="grid gap-3">
                <div>
                  <label className="text-[13px] font-bold uppercase tracking-wide text-[var(--text-light)]">Full name</label>
                  <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Administrator" className="mt-1.5 w-full rounded-[4px] border border-[var(--border)] bg-white dark:bg-[#1a221a] px-4 py-3 text-[16px] sm:text-[15px] outline-none focus:border-[#4a8c3f] focus:ring-4 focus:ring-[#4a8c3f]/10 transition-all" />
                </div>
                <div>
                  <label className="text-[13px] font-bold uppercase tracking-wide text-[var(--text-light)]">Email address</label>
                  <input value={emailField} onChange={(e) => setEmailField(e.target.value)} placeholder="lead@rich.africa" className="mt-1.5 w-full rounded-[4px] border border-[var(--border)] bg-white dark:bg-[#1a221a] px-4 py-3 text-[16px] sm:text-[15px] outline-none focus:border-[#4a8c3f] focus:ring-4 focus:ring-[#4a8c3f]/10 transition-all" />
                </div>
              </div>
              <button onClick={saveProfile} disabled={savingProfile} className="cursor-pointer w-full rounded-[4px] bg-[#1a3a1a] px-6 py-3 text-[15px] font-bold text-white hover:bg-[#2d5a27] disabled:opacity-60 transition-colors flex items-center justify-center gap-2">
                {savingProfile ? (<><svg className="h-5 w-5 animate-spin text-white" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-25" /><path d="M4 12a8 8 0 018-8" stroke="currentColor" strokeWidth="4" strokeLinecap="round" className="opacity-75" /></svg> Saving…</>) : "Save profile"}
              </button>
              ...
              <div className="border-t border-[var(--border)] pt-5">
                ...
                <button onClick={resetPw} disabled={savingPw} className="cursor-pointer mt-3 w-full rounded-[4px] border border-[var(--border)] bg-white dark:bg-white/5 px-6 py-3 text-[15px] font-bold text-[var(--text-dark)] hover:border-[#4a8c3f] hover:text-[#4a8c3f] disabled:opacity-60 transition-colors flex items-center justify-center gap-2">
                  {savingPw ? (<><svg className="h-5 w-5 animate-spin text-[var(--text-dark)]" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-25" /><path d="M4 12a8 8 0 018-8" stroke="currentColor" strokeWidth="4" strokeLinecap="round" className="opacity-75" /></svg> Updating…</>) : "Update password"}
                </button>
              </div>
            </div>
          </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-[640px] max-h-[92dvh] sm:max-h-[90vh] overflow-auto rounded-t-[16px] sm:rounded-[16px] bg-[#f7f6f4] dark:bg-[#0f1410] border border-[#e8ece8] dark:border-white/10 shadow-2xl animate-[toast-in_0.32s_ease] pb-[env(safe-area-inset-bottom)]">
            ...
            <div className="bg-white dark:bg-[#1a221a] p-4 sm:p-6 space-y-5">
              <div>
              ...
              <div className="rounded-[12px] bg-[var(--off-white)] dark:bg-white/5 border border-[var(--border)] p-4">
                ...
                <div className="mt-4 flex flex-col sm:flex-row gap-2">
                  <button onClick={() => act(selected.id, "published")} className="flex-1 rounded-[4px] bg-[#4a8c3f] py-3.5 sm:py-3 text-[15px] font-bold text-white hover:bg-[#2d5a27] hover:shadow transition-all touch-manipulation min-h-[44px]">Publish to repository</button>
                  <button onClick={() => { setDeclineTarget(selected); setDeclineNotes(selected.reviewNotes || ""); }} className="rounded-[4px] border border-red-200 bg-white px-4 py-3.5 sm:py-3 text-[15px] font-bold text-red-600 hover:bg-red-50 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300 transition-colors touch-manipulation min-h-[44px]">Decline</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {declineTarget && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-[560px] max-h-[92dvh] overflow-auto rounded-t-[16px] sm:rounded-[16px] bg-white dark:bg-[#1a221a] border border-[var(--border)] shadow-2xl animate-[toast-in_0.3s_ease] overflow-hidden pb-[env(safe-area-inset-bottom)]">
            <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-[var(--border)] flex items-start justify-between gap-4">
              <div>
            ...
            <div className="p-4 sm:p-6 space-y-4">
              <div>
                ...
                <textarea value={declineNotes} onChange={(e) => setDeclineNotes(e.target.value)} rows={4} placeholder="Explain why this is declined and what to improve - visible to the contributor in their dashboard." className="mt-2 w-full rounded-[8px] border border-[var(--border)] bg-white dark:bg-white/5 px-4 py-3 text-[16px] sm:text-[14px] leading-6 outline-none focus:border-red-400 focus:ring-4 focus:ring-red-100 dark:focus:ring-red-900/30" />
                <p className="mt-1.5 text-[12px] text-[var(--text-light)]">{declineNotes.length}/2000 • at least 10 characters</p>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setDeclineTarget(null)} className="flex-1 rounded-[4px] border border-[var(--border)] bg-white dark:bg-white/5 px-4 py-3.5 sm:py-3 text-[14px] font-bold text-[var(--text-dark)] hover:bg-[var(--off-white)] transition-colors touch-manipulation min-h-[44px]">Cancel</button>
                <button onClick={confirmDecline} disabled={declining || declineNotes.trim().length < 10} className="flex-1 rounded-[4px] bg-red-600 px-4 py-3.5 sm:py-3 text-[14px] font-bold text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors touch-manipulation min-h-[44px]">{declining ? "Declining…" : "Decline with notes"}</button>
              </div>
            </div>
          </div>
        </div>
