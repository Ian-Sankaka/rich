        ...
        <div className="flex-1 p-6 lg:p-8 space-y-6 bg-[#f8f8f5] dark:bg-[#070d07] overflow-auto">
          ...
          <motion.div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { staggerChildren: 0.1 } } }} initial="hidden" animate="visible">
            {[
              ...
              { label: "Pending", value: stats.pending, hint: "Needs review", sub: "Awaiting triage", icon: Clock, color: "bg-amber-500", pct: stats.total ? (stats.pending / stats.total) * 100 : 0 },
          ...
          <div className="relative w-full max-w-[560px] max-h-[90vh] overflow-auto rounded-[16px] bg-white dark:bg-[#1a221a] border border-[var(--border)] shadow-2xl animate-[toast-in_0.3s_ease]">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid gap-3">
                <div><label className="text-[14px] font-bold uppercase tracking-wide text-[var(--text-light)]">Full name</label><input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Your name" className="mt-1.5 w-full rounded-[4px] border border-[var(--border)] bg-white dark:bg-[#1a221a] px-4 py-3 text-[16px] outline-none focus:border-[#4a8c3f] focus:ring-4 focus:ring-[#4a8c3f]/10 transition-all" /></div>
                <div><label className="text-[14px] font-bold uppercase tracking-wide text-[var(--text-light)]">Email address</label><input value={emailField} onChange={(e) => setEmailField(e.target.value)} placeholder="you@rich.africa" className="mt-1.5 w-full rounded-[4px] border border-[var(--border)] bg-white dark:bg-[#1a221a] px-4 py-3 text-[16px] outline-none focus:border-[#4a8c3f] focus:ring-4 focus:ring-[#4a8c3f]/10 transition-all" /></div>
              </div>
              <button onClick={saveProfile} disabled={savingProfile} className="cursor-pointer w-full rounded-[4px] bg-[#1a3a1a] px-6 py-3 text-[16px] font-bold text-white hover:bg-[#2d5a27] disabled:opacity-60 transition-colors">{savingProfile ? "Saving…" : "Save profile"}</button>
              <div className="border-t border-[var(--border)] pt-5">
                ...
                <button onClick={resetPw} disabled={savingPw} className="cursor-pointer mt-3 w-full rounded-[4px] border border-[var(--border)] bg-white dark:bg-white/5 px-6 py-3 text-[16px] font-bold text-[var(--text-dark)] hover:border-[#4a8c3f] hover:text-[#4a8c3f] disabled:opacity-60 transition-colors">{savingPw ? "Updating…" : "Update password"}</button>
              </div>
            </div>
          </div>
        </div>
